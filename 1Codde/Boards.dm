...
[info]
	...
	proc/addMessage(mob/M)
		...
		messages[title]="<i>(Published [archive.day][Days(archive.day)] of [Months(archive.month)], [archive.year])</i><br><br>"+message
		if(messages.len>8)
			messages-=messages[9]
	...
	proc/approveGlobal(mob/M,message,title)
		...
		globalMessages[title]="<i>(Published [archive.day][Days(archive.day)] of [Months(archive.month)], [archive.year])</i><br><br>"+message
