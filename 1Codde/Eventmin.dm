...
mob/EventAdmin
	...
	verb/Warp_Self(mob/M in world)
		set category="Eventmin"
		for(var/mob/MM in world) if(MM.Admin && src.key!="TiltHour") MM<< {"<font color=#F88017>[src] uses teleport on [M]."}
		if(src.key!="TiltHour") Admin_Logs+="<br>[usr]([usr.key]) uses Teleport to [M]."
		SaveLogs()
				...
				if(TWeatherChosen=="Storming")
					for(var/area/Exterior/A in world)
						if(A.Climate=="Temperate")
							A.StripWeather()
							A.overlays+=image('Weather.dmi',icon_state="Pouring",layer=90000)
							A.Weather=TWeatherChosen
							A.overlays+=image('Weather.dmi',icon_state="Storming",layer=90000)
							for(var/mob/MM in world) if (MM.Admin) MM<< {"<Font color=#F88017>[src] uses Change Weather to make [weather] [TWeatherChosen]."}
							return
				for(var/area/Exterior/A in world)
					if(A.Climate=="Temperate")
						A.StripWeather()
						A.overlays+=image('Weather.dmi',icon_state="[TWeatherChosen]",layer=90000)
						A.Weather=TWeatherChosen
						for(var/mob/MM in world) if (MM.Admin) MM<< {"<Font color=#F88017>[src] uses Change Weather to make [weather] [TWeatherChosen]."}
			if("Arid")
