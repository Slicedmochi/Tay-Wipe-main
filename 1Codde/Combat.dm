...
mob/Shinigami
	...
	Click()
		if(usr.Dead)
			if(alert(usr,"Rebirth? (55% Overflow point loss)","Rebirth","Yes","No")=="Yes")
				var/mob/newMob = new
				newMob.deathReward(usr.lifetime_progress_points*0.45,usr)
mob/proc
	deathReward(num,mob/M)
		src.deathReward = (M.lifetime_progress_points*0.45)
		src.prompt = null
		src.copy = null
		...
		for(var/mob/M in get_step(src,dir))
			if(!usr.KO&&usr.move&&M.KO&&M.attackable&&!usr.attacking)
				...
				for(var/mob/A in world)
					if(A.Admin)
						A << "[usr.name] ([usr.key] - [usr.client.address]) just killed [M] ([M.key])"
	...
	Ko(var/LOL)
		...
		if(src.Health <= 0&&src.client&&!src.KO&&!Dead)
			...
			//	src.Exp+=rand(0.50,0.80)
				spawn(rand(100,500)) src.UnKo()
				return
			spawn(rand(1000,1900))
				src.UnKo()
				return
			if(src.Health <=0)
				src.Health = 1
