...
mob/Move(newloc,newdir) // Move proc
	...
	if(src.move) // If the move var is 1...
		if(incombat && !roaming)
			current_tiles++
			calculatemovement()
			if((get_dist(src.loc, incombat.Combat.loc)) > 65)
				_output("You have breached 65 tiles away from the source of combat, if this is legit, contact an admin.", "oocalert1", "outputall.output")
				return
		if(Invs)
			Invs.dir = dir
		if(grabbee!="")
		...
		if(loc) LastLoc=loc
		if(src.loc) if(istype(src.loc.loc,/area/Caves/))
			if(darkness) if(istype(darkness, /image)) darkness.alpha = 0
		else if(darkness) if(istype(darkness, /image)) if(darkness.alpha == 0) setLightOverlay(outside_light)
		if(Invs)
			Invs.appearance = appearance
	else // If its not 1...
