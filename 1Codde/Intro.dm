...
client
	MouseEntered(mob/M)
		...
		//if(!mob.TextName)
		if(istype(M,/mob/NPC) || istype(M, /mob/Clone))
			mob.TextName=image(null,M,null,layer=900)
			mob.TextName.pixel_y = -32
			mob.TextName.pixel_x = ((length(mob.name) * -4) / 2) + 11
			mob.TextName.layer=900
			var obj/o = drawfont.QuickText(src, "[M.name]", "#8d1cc2", 1, layer = 93000000)//M.nameInText
			o.plane = 99
