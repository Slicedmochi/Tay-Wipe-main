...
mob/proc/ChatLog(var/Info)
	...
	if(src.client)
		...
		if(fexists(outputFile))
			outputFile << "<br>[time2text(world.realtime, "DDD MMM DD hh:mm:ss YYYY")][Info] | (X:[x] Y:[y] Z:[z])"
		else
			outputFile << "<br>[time2text(world.realtime, "DDD MMM DD hh:mm:ss YYYY")][Info] | (X:[x] Y:[y] Z:[z])"
mob/proc/SelfLog(var/Info)
	...
	if(src.client)
		...
		if(fexists(outputFile))
			outputFile << "<br>[time2text(world.realtime, "DDD MMM DD hh:mm:ss YYYY")][Info]"
		else
			outputFile << "<br>[time2text(world.realtime, "DDD MMM DD hh:mm:ss YYYY")][Info]"
...
<font color="#00FFFF">Your logged activities<br>
	...
	Options()
		...
		//var/skin = src.CustomInput("Skin Selection","Please choose the skin color you would like",list("White","Tan","Dark","Pale"))
			//	switch(skin:name)
		var/list/choices = list("Subscribe","Change Short Description","IC Chat Color","Profile","Toggle Jump","Toggle OOC","Toggle Feats","Toggle Contacts","Cancel")
		if(getPerk("Armor-Smith") || getPerk("Weapon-Smith") || getPerk("Lotus Sealing") || getPerk("Pharmacist") || getPerk("Tailor") || getPerk("Herbalist"))
			choices = list("Subscribe","Change Short Description","IC Chat Color","Profile","Toggle Jump","Craft","Toggle OOC","Toggle Feats","Toggle Contacts","Cancel")
	...
	OOC(msg as text)
		...
		var/thetext=copytext(msg,1,150)
		OOCFont = "#858755" // OOC FONT COLORS
		...
		if(Donator)
			OOCFont = "#B52735"
		for(var/mob/M in ListOfPlayers())
			if(M.OOCon&&!M.Admin)
