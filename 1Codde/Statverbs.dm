...
*/
// lets convert grades to direct tile boosts
		...
		tile_movement()
			...
			for(var/mob/M in hearers(16,controlMob))
				M << output("<i>[M.getStrangerName(src)] can move [round(3 + GradeToTileBuff(get_grade_stat(speed+boost, 0 ), "run"))] tiles a turn at full sprint.</i>", "outputall.output")
				M << output("<i>[M.getStrangerName(src)] can move [round(2 + GradeToTileBuff(get_grade_stat(speed+boost, 0 ), "dodge"))] tiles a turn while dodging.</i>", "outputall.output")
				M << output("<i>[M.getStrangerName(src)] can move [round(1 + GradeToTileBuff(get_grade_stat(speed+boost, 0 ), "walk"))] tiles a turn while walking.</i>", "outputall.output")
		...
		strength()
			...
			switch(input("Hold back?") in list ("Yes","No"))
				if("No")
					...
					//var/amount = input(usr,"Your strength is [strength] + ([boost]) = [strength + boost]. Use how much strength?", "Use strength", (strength + boost)) as num
					//amount = min(strength + boost, amount)
					//using_strength = max(1, amount)
				...
				if("Yes")
					...
					if(value > 75) Letters += "A-"
					if(value > 63) Letters += "B+"
					if(value > 52) Letters += "B"
					if(value > 45) Letters += "B-"
...
/mob
	proc
		...
		strength_boost()
			...
			for(var/obj/jutsu/P in src)
				if(P.jutsu_type == "perk")
					if(P.boost_strength) boost += ( P.boost_strength * _get_step_size(get_grade_stat(src.strength)) )
			return boost
		...
		agility_boost()
			...
			for(var/obj/jutsu/P in src)
				if(P.jutsu_type == "perk" || (P.jutsu_type == "buff" && P.active))
					if(P.boost_agility) boost += ( P.boost_agility * _get_step_size(get_grade_stat(src.agility)) )
			return boost
		endurance_boost()
			var/boost = 0
			for(var/obj/jutsu/P in src)
				if(P.jutsu_type == "perk" || (P.jutsu_type == "buff" && P.active))
					if(P.boost_endurance) boost += (P.boost_endurance * _get_step_size(get_grade_stat(src.endurance)) )
			return boost
		...
		stamina_boost()
			...
			for(var/obj/jutsu/P in src)
				if(P.jutsu_type == "perk" || (P.jutsu_type == "buff" && P.active))
					if(P.boost_stamina) boost += (P.boost_stamina * _get_step_size(get_grade_stat(src.stamina)) )
			return boost
		...
		control_boost()
			...
			for(var/obj/jutsu/P in src)
				if(P.jutsu_type == "perk" || (P.jutsu_type == "buff" && P.active))
					if(P.boost_control) boost += (P.boost_control * _get_step_size(get_grade_stat(src.control)) )
			return boost
		...
		speed_boost()
			...
			for(var/obj/jutsu/P in src)
				if(P.jutsu_type == "perk" || (P.jutsu_type == "buff" && P.active))
					if(P.boost_speed) boost += ( P.boost_speed* _get_step_size(get_grade_stat(src.speed)) )
			return boost
		...
		chakra_boost_pool()
			...
			for(var/obj/jutsu/P in src)
				if(P.jutsu_type == "perk" || (P.jutsu_type == "buff" && P.active))
					if(P.boost_chakra_pool) boost += (P.boost_chakra_pool * _get_step_size(get_grade_stat((src.control + src.stamina) / 2)))
			return boost
		stamina_boost_pool()
			var/boost = 0
			for(var/obj/jutsu/P in src)
				if(P.jutsu_type == "perk" || (P.jutsu_type == "buff" && P.active))
					if(P.boost_stamina_pool) boost += (P.boost_stamina_pool * _get_step_size(get_grade_stat(src.stamina) ))
			return boost
		calories_boost_pool()
			var/boost = 0
			for(var/obj/jutsu/P in src)
