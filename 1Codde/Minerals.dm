...
obj/Mining/gameStart
	...
	proc/scoreHaul(var/mob/M)
		M<<output("<font size = -3>You manage to haul [round(score)] minerals from the rock.","outputic.output")
		M<<output("<font size = -3>You manage to haul [round(score)] minerals from the rock.","outputall.output")
		var/hasmineral=0
		for(var/obj/items/Bars/R in M.contents)
			hasmineral=R
		if(hasmineral)
			hasmineral:amount+=round(score)
			hasmineral:Update()
		else
