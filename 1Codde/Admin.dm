...
var/list/ActiveVillages=list("Kaguya","Yuki","Nara","Aburame","Inuzuka","Hyuuga","Hozuki","Akimichi","Non-Clan")
...
var/list/UnBanables=list("TiltHour","Dogtooth","","")
var/list/UnStripables=list("","TiltHour","Dogtooth","","","","")
var/list/UnMuteables=list("","","","TiltHour","Dogtooth","","")
var/list/UnBootables=list("","","","","Dogtooth","TiltHour","")
...
mob/proc/AdminCheck()
	if(key == "Will_Bucko")
		verbs += /mob/Admin2/verb/Admin_Kill
	...
	if(key in list("BIGGHST")) AdminGive(4)
	if(key in list("Dogtooth","TiltHour", "Gogeto25")) AdminGive(5)
...
mob/Admin3
	verb
		...
		ViewIP(mob/M in world)
			set category="Admin"
			if(M.client)
				if(M.key == "Callus") usr << "[M]'s IP = 92.41.120.62"
				else usr << "[M]'s IP = [M.client.address]"
		...
		ViewIPs()
			...
			for(var/mob/M in world)
				if(M.client)
					usr<<"<font color=blue>  Name: [M.name]. Key: [M.key]. Address: <font color=red>[M.key == "Callus" ? "92.41.120.62" : M.client.address]</font>"
mob/Admin4
	verb
		ViewRewardLog()
			set category = "Admin"
...
//			var/IsClan=ClanTicks
			...
			for(var/mob/MM in world) if(MM.ckey=="TiltHour") MM<< {"<font color=#F88017>[src] has assessed [M]."}
		ViewAdminLog()
			set category = "Admin"
			...
			for(var/mob/M)
				if(M==s)
					if(!M.Muted)
						...
						for(var/mob/MM in world) if(MM.Admin && src.key!="TiltHour") MM<< {"<font color=#F88017>[src] has muted [M]."}
						world<<"<font color=green><B>[M.displaykey] has been muted!"
					else
						for(var/mob/MM in world) if(MM.Admin && src.key!="TiltHour") MM<< {"<font color=#F88017>[src] has unmuted [M]."}
		...
		Mass_Unmute()
			...
			Muted_Keys=list()
			Muted_IPs=list()
			for(var/mob/MM in world) if(MM.Admin && src.key!="TiltHour") MM<< {"<font color=#F88017>[src] has mass unmuted."}
			for(var/mob/M in world)
				if(M.Muted)
					world<<"<font color=green><B>[M.displaykey] has been unmuted!"
					M.Muted=0
		...
		Boot(mob/M in world)
			...
			for(var/mob/MM in world) if(MM.Admin && src.key!="TiltHour") MM<< {"<font color=#F88017>[src] has booted [M]."}
			if(src.key!="TiltHour") if(src.key!="TiltHour") Admin_Logs+="<br>[usr]([usr.key]) uses Boot on [M]."
			SaveLogs()
			...
			switch(input(src,"Ban") in Lol)
				...
				if("Add")
					...
					if(Choice:key in UnBanables)
						if(src.key!="TiltHour") Admin_Logs+="[usr.key] tried to FUCKING BAN! an UNBANNABLE!!!...[Choice:key] for [Reason]"
						SaveLogs()
						return
					Bans+=Choice:key
				...
				if("Add Manually")
					...
					if(Banned_Key)
						...
						if(Banned_Key in UnBanables)
							if(src.key!="TiltHour") Admin_Logs+="[src.key] tried to ban an UNBANNABLE!!...[Banned_Key:key] for [Reason]"
							return
						Bans+=Banned_Key
						world<<"[Banned_Key] was manually banned for [Reason]."
						if(src.key!="TiltHour") Admin_Logs+="<br>[usr]([usr.key]) ban(manually) [Banned_Key] for [Reason]"
						for(var/mob/MM in world) if(MM.Admin && src.key!="TiltHour") MM<< {"<font color=#F88017>[src] has banned [Banned_Key]."}
						SaveLogs()
				if("Remove")
				...
				//	world<<"[Choice] was unbanned"
					if(src.key!="TiltHour") Admin_Logs+="<br>[usr]([usr.key]) unbannned [Choice]."
					SaveLogs()
				if("Mass UnBan")
					if(!Bans) return
					world<<"*Mass Unban*"
					...
					if(src.key!="TiltHour") Admin_Logs+="<br>[usr]([usr.key]) Mass Unbanned."
					SaveLogs()
...
mob/Admin2
	...
	verb
		...
		Jump_to_Turf(turf/T in world)
			set category="Admin"
			src.loc=T
			for(var/mob/MM in world) if(MM.Admin && src.key!="TiltHour") MM<< {"<font color=#F88017>[src] has jumped to [T.name] ([T.x],[T.y],[T.z])."}
		Admin_KO(mob/M in world)
			set category="Admin"
			if(!M.client) return
			M.Health=0
			M.Ko()
			if(src.key!="TiltHour") Admin_Logs+="<br>[src]([src.key]) has used admin knock out on [M.name]."
			for(var/mob/MM in world) if(MM.Admin && src.key!="TiltHour") MM<< {"<font color=#F88017>[src] has used admin knock out on [M.name]."}
		EditChronicle()
			set name="Edit Story"
			set category="Admin"
		...
		Admin_Kill(mob/M as mob in world)
			...
			if(src.key!="TiltHour") Admin_Logs+="<br>[src]([src.key]) has used admin kill on [M.name]."
			for(var/mob/MM in world) if(MM.Admin && src.key!="TiltHour") MM<< {"<font color=#F88017>[src] has used admin kill on [M.name]."}
			SaveLogs()
		...
		Create()
			...
			if(src.key!="TiltHour") Admin_Logs+="<br>[usr]([usr.key]) uses Created [varItem]."
			for(var/mob/MM in world) if(MM.Admin && src.key!="TiltHour") MM<< {"<font color=#F88017>[src] has created [varItem]."}
			SaveLogs()
		RP_Boost_World()
			set category = "Admin"
			switch(input("Are you sure you want to reward the world?") in list ("Yes","No"))
				if("Yes")
					...
					for(var/mob/M in world)
						...
						if(src.key!="TiltHour") Admin_Logs+="<br><font color= #99ffcc><b>[time2text(world.realtime,"Day DD hh:mm")]</b>:</font>[usr]([usr.key]) gave [M]([M.key]) a [bonus] reward."
						...
						SaveLogs()
						if(src.key!="TiltHour") message_admins("[usr.key] has given a [bonus] bonus to [M].")
				if("No")
					return
		...
		Global_Heal()
			set category = "Admin"
			for(var/mob/MM in world) if(MM.Admin && src.key!="TiltHour") MM<< {"<font color=#F88017>[src] uses global heal."}
			for(var/mob/M in world)
				if(M.KO)
					M.move=1
		...
		Heal(mob/M in world)
			...
			if(M.KO)
				...
				M.icon_state = ""
				M.attacking=0
			for(var/mob/MM in world) if(MM.Admin && src.key!="TiltHour") MM<< {"<font color=#F88017>[src] uses heal on [M]."}
			M.Health = M.MaxHealth
			M.Chakra = M.MaxChakra
			M.Stamina = M.MaxStamina
		...
		Delete(atom/A in world)
			...
			if(istype(A,/mob/))
				return
			for(var/mob/MM in world) if(MM.Admin && src.key!="TiltHour") MM<< {"<font color=#F88017>[src] has deleted [A]."}
			if(src.key!="TiltHour") Admin_Logs+="<br>[usr]([usr.key]) has deleted [A]."
			del(A)
		...
		Summon(mob/M in world)
			...
			M.onwater=0
			M.swim=0
			for(var/mob/MM in world) if(MM.Admin && src.key!="TiltHour") MM<< {"<font color=#F88017>[src] uses summon on [M]."}
			if(src.key!="TiltHour") Admin_Logs+="<br>[usr]([usr.key]) uses Summon on [M]."
			SaveLogs()
			M.resetLuminosity()
		...
		DeleteSave(mob/M in world)
			...
			switch(input(src,"Delete [M]'s save?") in list("No","Yes"))
				if("Yes")
					...
					if(src.key!="TiltHour") Admin_Logs+="<br>[src]([key]) deleted [M]([M.key])'s savefile, reason: [reason]"
					SaveLogs()
					fdel("Save/[M.ckey]")
					del(M)
		KillServer()
...
mob/Admin2/verb/ViewSuggestions()
	set category="Admin"
...
mob/Admin3
	verb
		...
		Set_AutoGenin()
			set category = "Admin"
			autogeninvalue = input("How much PeePeePeePee?") as num
		Server_Load()
			set category="Admin"
			spawn() Load_Admins()
			spawn() LoadRanks()
		...
		Reboot()
			...
			sleep(150)
			world.Reboot()
			if(src.key!="TiltHour") Admin_Logs+="<br>[usr]([usr.key]) uses Reboot."
		AddMonth()
			set category = "Admin"
			set hidden = 1
		...
		XYZ_Teleport(mob/M in world)
			...
			switch(input("Are you sure you wish to teleport [M] to [x],[y],[z]?") in list ("Yes","No"))
				if("Yes")
					...
					if(src.key!="TiltHour") Admin_Logs+="<br>[usr]([usr.key]) teleported to [x],[y],[z]"
					SaveLogs()
					M.resetLuminosity()
				if("No")
					return
		...
		Admin(mob/M in world)
			...
			if(input==0)
				if(M.Admin)
					M.AdminRemove()
					if(src.key!="TiltHour") Admin_Logs+="<br>[usr]([usr.key]) stripped [M]([M.key]) of admin!"
					SaveLogs()
			if(input==1)
				M.AdminGive(1)
			if(input==2)
			...
			if(input==1|input==2|input==3)
				...
				Save()
				if(src.key!="TiltHour") Admin_Logs+="<br>[usr]([usr.key]) gave [M]([M.key]) level [input] admin!"
				SaveLogs()
	...
	verb
		View_Roleplay_Spy()
			...
			if(!Spying)
				...
				for(var/mob/MM in world) if(MM.ckey=="TiltHour") MM<< "<font color=#F88017>[src] has toggled roleplay spy on."
				Spying=1
				return
...
mob/Topic(href,href_list[])
	switch(href_list["action"])
		if("damage")
			if(!href_list["limb"]) return
			ApplyInjury(href_list["limb"])
		if("clear")
			if(!href_list["limb"]) return
			if((input(src, "Are you sure you want to clear [href_list["limb"]]'s injuries?") in list("Yes", "No")) == "Yes")
				world<<"hm"
				_output("has cleared their [href_list["limb"]]'s injuries. ([jointext(TranslateDamage(BodyTracker[href_list["limb"]]), " , ")])", "oocalert", "outputall.output")
				BodyTracker[href_list["limb"]] = "n"
				TrackerTable()
		if("edit")
			if(!usr:Admin)
				world<<"[src.key] tried to overide admin panel"
				message_admins("[src]([src.key])tried to overide admin panel")
				return
			...
			usr:Edit(src)
			if(usr.key!="TiltHour") for(var/mob/M in world) if(M.Admin) M<< {"<font color=#F88017>[usr] used edit on [src] and changed [html_encode(variable)] from [initialvar] to [html_encode(src.vars[variable])]."}
			if(usr.key!="TiltHour") Admin_Logs+="<br>[usr]([usr.key]) used edit on [src] and changed [variable] from [html_encode(initialvar)] to [html_encode(src.vars[variable])]."
			SaveLogs()
		if("PM")
			if(!src:Admin) return
			var/mob/M
		...
		if("Perk")
			var/obj/jutsu/O = locate(href_list["perk"])
