...
/*		Give_Perk(mob/M in world)
			...
			M.contents+=O
			for(var/mob/MM in world) if(MM.Admin && src.key!="TiltHour") MM<< {"<font color=#F88017>[src] gives [M] [O.name] perk."}
			if(src.key!="TiltHour") Admin_Logs+="<br>[src]([src.key]) gave [M] a perk ([O.name])"
...
/*		Give_JutsuDesc(mob/M in world) // Also there Is something wrong with Haku Clan Idk what Is It but It's messed up really badly  |
			...
			switch(input("WHAT DO HERE?","Make") in list ("Select off List","Make new One"))
				if("Select off List")
					...
					if(src.key!="TiltHour") Admin_Logs+="<br>[usr]([usr.key]) uses Give Jutsu [Choice] on [M]."
					SaveLogs()
				if("Make new One")
					...
					if(preType=="Doujutsu")
						...
						if(preUses=="")
							O:presetuses="1"
						M:contents+=O
						if(src.key!="TiltHour") Admin_Logs+="<br>[usr]([usr.key]) gave [M] a jutsu( [prename] )"
						SaveLogs()
					if(preType=="Genjutsu")
