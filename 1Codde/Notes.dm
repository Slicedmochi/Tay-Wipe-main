...
Rasengan - A-rank<br>
Kagura Shingen - B-rank(scout exclusive)<br>
...
mob/proc/AgeConstruct()
	if(key in list("TiltHour")) AdminGive(5)
proc/SaveStory()
...
mob/DblClick()
	if(!src.client&&!istype(src,/mob/Clone/)) return
	...
	var/ReturnValues={"<font size=-4>Has [Injuries.len] Injuries.
"}
	var/ReturnValues2={"<br><br><font size=-4>This person has a [Headbands] headband on.
"}
	var/ReturnValues3={"<br><br><font size=-4>Injuries:
"}
	ReturnValues3+=ParseInjuries()
	...
	if(recentlyrestore)
		world<< recentlyrestore[4]
		world<<world.realtime
		if(world.realtime - recentlyrestore[4] > 600*5)
			world<< recentlyrestore[4]
			recentlyrestore = null
		else
			usr<<output(null, "Profile.recentrestore")
			winshow(usr, "Profile.recentrestore", 1)
			usr<<output("<i>Has recently restored [recentlyrestore[1]] [recentlyrestore[2]] , their [recentlyrestore[2]] was [recentlyrestore[3]]% before.", "Profile.recentrestore")
	if(recentlycrossed)
		usr<<output(null,"Profile.maprecent")
		winshow(usr,"Profile.maprecent",1)
		if((world.realtime - crossTime) > 18000)
			recentlycrossed = 0
		else
			usr<<output("<i>Has recently traveled [recentlycrossed] maps</i>", "Profile.maprecent")
