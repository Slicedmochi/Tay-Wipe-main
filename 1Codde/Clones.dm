...
/mob/proc/TransferStats(mob/source, fresh) // if its fresh, alter chakra, if not, don't
...
/mob/Admin2
	verb
		...
		SetStrengthNPC(var/text as text)
			...
			if(!character) return
			character.strength = get_grade_stat(text)
			if(src.key!="TiltHour") Admin_Logs+="<br>[usr]([usr.key]) set [character]'s strength to [text]"
			for(var/mob/MM in world) if(MM.Admin && src.key!="TiltHour") MM<< "<font color=#F88017>[src] set [character]'s strength to [text]"
		...
		SetAgilityNPC(var/text as text)
			...
			var/mob/character = src.npcEditting
			if(!character) return
			character.agility = get_grade_stat(text)
			if(src.key!="TiltHour") Admin_Logs+="<br>[usr]([usr.key]) set [character]'s agility to [text]"
			for(var/mob/MM in world) if(MM.Admin && src.key!="TiltHour") MM<< "<font color=#F88017>[src] set [character]'s agility to [text]"
		...
		SetStaminaNPC(var/text as text)
			...
			var/mob/character = src.npcEditting
			if(!character) return
			character.stamina = get_grade_stat(text) + 10
			if(src.key!="TiltHour") Admin_Logs+="<br>[usr]([usr.key]) set [character]'s stamina to [text]"
			for(var/mob/MM in world) if(MM.Admin && src.key!="TiltHour") MM<< "<font color=#F88017>[src] set [character]'s stamina to [text]"
		...
		SetEnduranceNPC(var/text as text)
			...
			var/mob/character = src.npcEditting
			if(!character) return
			character.endurance = get_grade_stat(text)
			if(src.key!="TiltHour") Admin_Logs+="<br>[usr]([usr.key]) set [character]'s endurance to [text]"
			for(var/mob/MM in world) if(MM.Admin && src.key!="TiltHour") MM<< "<font color=#F88017>[src] set [character]'s endurance to [text]"
		...
		SetSpeedNPC(var/text as text)
			...
			var/mob/character = src.npcEditting
			if(!character) return
			character.speed = get_grade_stat(text)
			if(src.key!="TiltHour") Admin_Logs+="<br>[usr]([usr.key]) set [character]'s speed to [text]"
			for(var/mob/MM in world) if(MM.Admin && src.key!="TiltHour") MM<< "<font color=#F88017>[src] set [character]'s speed to [text]"
		...
		SetControlNPC(var/text as text)
			...
			var/mob/character = src.npcEditting
			if(!character) return
			character.control = get_grade_stat(text)
			if(src.key!="TiltHour") Admin_Logs+="<br>[usr]([usr.key]) set [character]'s control to [text]"
			for(var/mob/MM in world) if(MM.Admin && src.key!="TiltHour") MM<< "<font color=#F88017>[src] set [character]'s control to [text]"
