			...
			if(usr.Sharingan)
				if(usr.move)
					...
					if(usr.Genjutsu >= A.Genjutsu*1.1)
						...
						usr<<output("<font size = -3>[A] resists your Genjutsu!","ICout")
		...
		if(src in usr.contents)
			if(usr.move)
				...
				if(prob(10/skill/skill/skill))
					usr<<output("Your understanding of the Ayatsuito no Jutsu has increased.","ICout")
					skill+=1
					Update()
				usr.firing=1
				src.Active=1
				...
				viewers(16)<<output("<font color=[usr.SayFont]>[usr] yells<font color = white>: Ayatsuito no Jutsu!","ICout")
				usr.Jutsu_Gain()
				A.loc=usr.loc
				A.dir = usr.dir
				walk(A,usr.dir)
				...
				if("Totsuka's Sword")
					...
					if(usr.move)
						...
						for(var/mob/M in oview(2,usr))
							if(A)
								if(M.loc==A.loc)
									var/damage=(usr.Ninjutsu/M.Defence+M.Resistance)*rand(4,6)
									// M.Health -= damage
									M.Ko()
						spawn(45)
							usr.firing=0
	...
	Click()
		if(src in usr.contents)
			for(var/mob/Susanoo/Susanoo/E in world)
				if(E.Owner==usr.key)
					usr<<output("<font size = -3>You already summoned Susanoo.","ICout")
					return
			...
			if(!usr.move)
				...
				if(prob(10/Kuchiyoseskill/Kuchiyoseskill/Kuchiyoseskill))
					usr<<output("<font size = -3>Your understanding of Susanoo has increased.","ICout")
					Kuchiyoseskill+=1
					Update()
			usr.Jutsu_Gain()
		...
		if(src in usr.contents)
			if(usr.move)
				...
				if(usr.Chakra<400)
					return
				if(prob(10/skill/skill/skill))
					usr<<output("<font size = -3>Your understanding of Amateratsu has increased.","ICout")
					skill+=0.5
				usr.firing=1
				src.Active=1
				usr.MaxNinjutsu += (1 *0.001)
				...
				viewers(16)<<output("<font size = -3><font color=[usr.SayFont]>[usr] yells<font color = white>: Amaterasu!","ICout")
				usr.Jutsu_Gain()
				if(usr.dir==NORTH)
					A.loc = locate(usr.x,usr.y+1,usr.z)
	...
	Click()
		if(src in usr.contents)
			if(usr.Health<10)
				return
			if(usr.Chakra<100)
				usr<<output("<font size = -3>Your Chakra is to low to take out the flames of Amaterasu.","ICout")
				return
			if(!usr.Ama)
				return
			usr.Ama=0
			usr.Health-=5
			usr.overlays+='Bleed.dmi'
			usr<<output("<font size = -3>You took out the flames of Amaterasu.","ICout")
			...
			for(var/mob/M in oview(10))
				...
				if(M.Burn)
					...
					usr<<output("<font size = -3>The black flames around [M] has disappeared!","ICout")
					M.Burn=0
					M<<output("<font size = -3>The black flames around you has disappeared!","ICout")
		...
		if(src in usr.contents)
			if(usr.move)
				...
				if(A.Mastered)
					...
					A.move=0
					A.icon_state="KO"
					viewers(16)<<output("<font size = -3><font color=[usr.SayFont]>[usr] yells<font color = white>: Tsukuyomi!","ICout")
					A.MaxGenjutsu += (1 *0.01)
					sleep(50)
					A.move=1
					A.icon_state=""
					A<<output("<font size = -3>[A] countered your Genjutsu!","ICout")
					usr.Health-=10
					usr.Tsukuyomi=0
					return
				if(!A.meditate)
