		...
		if(src in usr.contents)
			if(!usr.Chidori)
				...
				if(prob(10/skill/skill/skill))
					usr<<output("<font size = -3>Your understanding of the Raiton: Chidori has increased.","ICout")
					usr<<output("<font size = -3>Your understanding of the Raiton: Chidori has increased.","ICOOCout")
					skill+=1
					Update()
		...
		if(src in usr.contents)
			if(usr.move)
				...
				if(usr.Chakra<usr.MaxChakra*(0.45/skill))
					return
				if(prob(10/skill/skill/skill))
					usr<<output("<font size = -3>Your understanding of the Chidori Senbon no Jutsu has increased.","ICout")
					usr<<output("<font size = -3>Your understanding of the Chidori Senbon no Jutsu has increased.","ICOOCout")
					skill+=1
					Update()
				usr.firing=1
				src.Active=1
