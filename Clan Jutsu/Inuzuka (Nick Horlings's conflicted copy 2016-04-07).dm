			...
			switch(input("Choose Option","",text) in Choices)
				...
				if("Juujin Bunshin")
					...
					if(!src.Juujin)
						...
						src.overlays = null
						src.name = src.Oname
						nameInText = cinefont.QuickName(name,"#fff",1,0,3,9000000)
						src.Juujin = 0
						src.icon = 'Dog.dmi'
	...
	Click()
		if(src in usr.contents)
			...
			for(var/mob/Dog/D in world)
				if(D.Owner==usr.key)
					usr<<output("<font size = -3>You may only summon one Dog at a time.","ICout")
					usr<<output("<font size = -3>You may only summon one Dog at a time.","ICOOCout")
					return
			usr.Chakra-=50
			Active=1
			spawn(50) Active=0
	...
	Click()
		if(src in usr.contents)
			...
			if(locate(/obj/items/Weapon/Enma_Pole,usr.contents))
				usr<<output("<font size = -3>You have already summoned King Enma.","ICout")
				usr<<output("<font size = -3>You have already summoned King Enma.","ICOOCout")
				return
	...
	Click()
		if(src in usr.contents)
			...
			for(var/mob/Slug/D in world)
				if(D.Owner==usr.key)
					usr<<output("<font size = -3>You may only summon one Slug at a time.","ICout")
					usr<<output("<font size = -3>You may only summon one Slug at a time.","ICOOCout")
					return
			usr.Chakra-=50
			Active=1
			spawn(50) Active=0
	...
	Click()
		if(src in usr.contents)
			...
			for(var/mob/Toad/D in world)
				if(D.Owner==usr.key)
					usr<<output("<font size = -3>You may only summon one Toad at a time.","ICout")
					usr<<output("<font size = -3>You may only summon one Toad at a time.","ICOOCout")
					return
			usr.Chakra-=50
			Active=1
			spawn(50) Active=0
	...
	Click()
		if(src in usr.contents)
			for(var/mob/Big_Snake/Big_Snake/E in world)
				if(E.Owner==usr.key)
					usr<<output("<font size = -3>You may only summon one Snake at a time.","ICout")
					usr<<output("<font size = -3>You may only summon one Snake at a time.","ICOOCout")
					return
			if(usr.Chakra<usr.MaxChakra*(0.80/Kuchiyoseskill))
				return
			if(!usr.move)
	...
	Click()
		if(src in usr.contents)
			...
			for(var/mob/Wolf/W in world)
				if(W.Owner==usr.key)
					usr<<output("<font size = -3>You may only summon one Wolf at a time.","ICout")
					usr<<output("<font size = -3>You may only summon one Wolf at a time.","ICOOCout")
					return
			usr.Chakra-=70
			Active=1
			spawn(50) Active=0
	...
	Click()
		if(src in usr.contents)
			...
			for(var/mob/Cat/C in world)
				if(C.Owner==usr.key)
					usr<<output("<font size = -3>You may only summon one Cat at a time.","ICout")
					usr<<output("<font size = -3>You may only summon one Cat at a time.","ICOOCout")
					return
			usr.Chakra-=50
			Active=1
			spawn(50) Active=0
