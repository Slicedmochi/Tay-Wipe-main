	...
	Click()
		if(src in usr.contents)
			for(var/mob/Bijuu/Shukaku/E in world)
				if(E.Owner==usr.key)
					usr<<output("<font size = -3>You already summoned Shukaku.","ICout")
					usr<<output("<font size = -3>You already summoned Shukaku.","ICOOCout")
					return
			...
			if(!usr.move)
				...
				if(prob(10/Kuchiyoseskill/Kuchiyoseskill/Kuchiyoseskill))
					usr<<output("<font size = -3>Your understanding of the Tanukineiri no Jutsu has increased.","ICout")
					usr<<output("<font size = -3>Your understanding of the Tanukineiri no Jutsu has increased.","ICOOCout")
					Kuchiyoseskill+=1
					Update()
			usr.Jutsu_Gain()
		...
		if(src in usr.contents)
			if(usr.move)
				...
				if(usr.Ninjutsu >= A.Resistance*1.1)
					...
					usr<<output("<font size = -3>[A] resists your Sabaku Kyuu!","ICout")
					usr<<output("<font size = -3>[A] resists your Sabaku Kyuu!","ICOOCout")
		...
		if(src in usr.contents)
			if(usr.move)
				...
				if(usr.Ninjutsu >= A.Resistance*1.1)
					...
					usr<<output("<font size = -3>[A] resists your Sabaku Sousou!","ICout")
					usr<<output("<font size = -3>[A] resists your Sabaku Sousou!","ICOOCout")
		...
		if(src in usr.contents)
			if(!Mayu)
				...
				if(prob(10/skill/skill/skill))
					usr<<output("<font size = -3>Your understanding of Suna no Mayu has increased.","ICout")
					skill+=1
					Update()
		...
		if(src in usr.contents)
			if(!Yoroi)
				if(usr.Chakra<usr.MaxChakra*0.10)
					return
				if(prob(10/skill/skill/skill))
					usr<<output("<font size = -3>Your understanding of Suna no Yoroi has increased.","ICout")
					usr<<output("<font size = -3>Your understanding of Suna no Yoroi has increased.","ICOOCout")
					skill+=1
					Update()
				...
				Yoroi=0
				src.Active=0
				usr<<output("<font size = -3>You have deactivated Suna no Yoroi","ICout")
				usr<<output("<font size = -3>You have deactivated Suna no Yoroi","ICOOCout")
		...
		if(src in usr.contents)
			if(!usr.Daisan)
				...
				if(usr.Chakra<usr.MaxChakra*(0.10/skill))
					return
				if(prob(10/skill/skill/skill))
					usr<<output("<font size = -3>Your understanding of Daisan no Me has increased.","ICout")
					usr<<output("<font size = -3>Your understanding of Daisan no Me has increased.","ICOOCout")
					skill+=1
					Update()
				usr.firing=1
				src.Active=1
