	...
	Click()
		if(src in usr.contents)
			for(var/mob/Bugs/Bugs/R in world)
				if(R.Owner==usr.key)
					usr << output("<font size = -3>You may only summon one swarm of Bugs at a time.","ICout")
					usr << output("<font size = -3>You may only summon one swarm of Bugs at a time.","ICOOCout")
					return
		...
		if(src in usr.contents)
			if(usr.move)
				...
				if(prob(10/skill/skill/skill))
					usr<<output("<font size = -3>Your understanding of the Kikaichuu no Jutsu has increased.","ICout")
					usr<<output("<font size = -3>Your understanding of the Kikaichuu no Jutsu has increased.","ICOOCout")
					Update()
					skill+=1
				usr.firing=1
				src.Active=1
		...
		if(src in usr.contents)
			if(usr.move)
				...
				if(usr.Ninjutsu >= A.Ninjutsu*1.1)
					...
					usr << output("<font size = -3>[A] resists your Mushi Dama!","ICout")
					usr << output("<font size = -3>[A] resists your Mushi Dama!","ICOOCout")
