		...
		if(src in usr.contents)
			if(!Nikudan)
				...
				if(prob(10/skill/skill/skill))
					usr<<output("<font size = -3>Your understanding of the Nikudan Sensha has increased.","ICout")
					usr<<output("<font size = -3>Your understanding of the Nikudan Sensha has increased.","ICOOCout")
					skill+=1
					Update()
		...
		if(src in usr.contents)
			if(!Choudan&&usr.Red)
				if(usr.Chakra<99)
					return
				if(prob(10/skill/skill/skill))
					usr<<output("<font size = -3>Your understanding of the Choudan Bakugeki has increased.","ICout")
					usr<<output("<font size = -3>Your understanding of the Choudan Bakugeki has increased.","ICOOCout")
					skill+=1
					Update()
				usr.MaxTaijutsu += (1 *0.001)
				usr.Chakra-=170
		...
		if(src in usr.contents)
			if(!Bubun)
				if(usr.Chakra<60)
					return
				if(prob(10/skill/skill/skill))
					usr<<output("<font size = -3>Your understanding of the Bubun Baika no Jutsu has increased.","ICout")
					usr<<output("<font size = -3>Your understanding of the Bubun Baika no Jutsu has increased.","ICOOCout")
					skill+=1
					Update()
				usr.MaxTaijutsu += (1 *0.001)
				usr.Chakra-=60
				...
				usr<<output("<font size = -3>Your have activated Bubun Baika no Jutsu.","ICout")
				usr<<output("<font size = -3>Your have activated Bubun Baika no Jutsu.","ICOOCout")
				src.Active=1
				Bubun=1
			else
				usr.Taijutsu = usr.MaxTaijutsu
				usr.overlays -= 'Bubun.dmi'
				usr<<output("<font size = -3>Your have deactivated Bubun Baika no Jutsu.","ICout")
				usr<<output("<font size = -3>Your have deactivated Bubun Baika no Jutsu.","ICOOCout")
				Bubun=0
				src.Active=0
	...
	Click()
		if(src in usr.contents)
			...
			for(var/obj/items/Clothing/C in usr.contents)
				if(C.worn)
					usr.overlays += C.icon
			usr.overlays += usr.underhair
			usr.overlays += usr.overhair
			usr.name = usr.Oname

			usr.nameInText = cinefont.QuickName(usr.name,"#fff",1,0,3,9000000)
