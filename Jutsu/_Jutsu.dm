...
/*		chakra_sight_x
	...
	proc/display_desc(mob/user)
		...
		for(var/mob/M in hearers(16, user))
			if(M.MindTransfer) if(M == M.MindTransfer.MindAfflicted) continue
			if(M.MindAfflicted)
				M.MindAfflicted << output("<font size=-2><font color=[user.SayFont]>[M.MindAfflicted.getStrangerName(user)] has activated a [jutsu_type != "perk" ? "technique" : "perk"]: <a href=?src=\ref[user];action=Perk;perk=[jutsuref]>[name]</a>!</font>", "outputall.output")
			else
				M << output("<font size=-2><font color=[user.SayFont]>[M.getStrangerName(user)] has activated a [jutsu_type != "perk" ? "technique" : "perk"]: <a href=?src=\ref[user];action=Perk;perk=[jutsuref]>[name]</a>!</font>", "outputall.output")

mob/var/list/chakra_images = list()
mob/var/byak_range_enhance = 0
/proc/findArchivePerk(n)
